...
data_curve_points_adjusted <- data_curve_points |> 
  group_by(FileName) |> 
  mutate(
    X_A = X[Point == "A"],
    Y_A = Y[Point == "A"],
...
# Kurven plotten
ggplot(bezier_df, aes(x = X, y = Y, group = FileName, color = FileName, linetype = Species)) +
  geom_path(linewidth = 0.5, alpha = 0.75) +
  coord_equal() +
  theme_minimal() +
...
# Kurven plotten
ggplot(bezier_df, aes(x = X, y = Y, group = FileName, color = Species, linetype = FileName)) +
  geom_path(linewidth = 0.75, alpha = 0.5) +
  coord_equal() +
  theme_minimal() +
  theme(legend.position = 'bottom') +
