...
output:
  pdf_document: 
    ...
    fig_caption: yes
    extra_dependencies: ["float", "acronym"]
...
{\Huge Shape optimisation of winged nuts (samaras), using maple species as an example} \\
...
\begin{acronym}
    \acro{l}[L.]{Carl von \underline{Linné}}
\end{acronym}
...
A digital camera (Panasonic Lumix G, Model No. DC-FZ100002) was secured on a tripod stand, pointed down and leveled via spirit level. Distance from ground to camera lens was measured 134cm. The following settings were chosen to take photos: Focal length 400, S 4, F 11, ISO 125; manual mode.  
50 photos for _Acer pseudoplatanus_ and _A. campestre_ and 100 photos for _A. platanoides_, 50 for each of the two samples were taken by remote control with the Panasonic Image App. The photos were copied to a Laptop and backed up to cloud storage and an external SSD.  
(Photos of the setup were taken, see below ==to be done==)

### Samples

1. _Acer campestre_ - Loki Schmidt Garten, Hamburg - what three words: ///verbunden.ergriffen.umwandeln
2. _Acer pseudoplatanus_ - Stadtpark, Hamburg - what three words: ///halbzeit.hebamme.forschte
3. _Acer platanoides_ - Spannskamp, Hamburg - what three words: ///vortritt.erkannt.teig
...
```{r feret_width_three_acer_species, fig.cap="\\label{fig:feret_width_three_acer_species}Feret lengths and maximum widths of three Acer species."}
...
```{r feret_width_two_platanoides_samples, fig.cap="\\label{fig:feret_width_two_platanoides_samples}Feret lengths and maximum widths of two samples of Acer platanoides."}
...
```{r feret_width_four_acer_samples, fig.cap="\\label{fig:feret_width_four_acer_samples}Feret lengths and maximum widths of four samples of Acer species."}
my_data_longer |> ggplot(
  mapping = aes(
    x = label,
    y = value,
...
```{r ratios_boxplot, fig.cap="\\label{fig:ratios_boxplot}Length/width ratios of four samples of Acer species"}
my_data |> ggplot(
  mapping = aes(
    x = label,
    y = ratio,
...
```{r ratios_hist, fig.cap="\\label{fig:ratios_hist}Length/width ratios of four samples of Acer species"}
my_data |> ggplot(
  mapping = aes(
    x = ratio
  )
